...
const getActivePremiere = async (req, res) => {
  try {
    ...
    // Premieres are NOT auto-ended when endTime passes. The video player
    // loops the stream and only the admin can end the premiere (via the
    // socket admin-end-premiere event or the REST end endpoint). endTime is
    // kept around as an informational original-schedule value.
    res.json({
      success: true,
      data: { premiere }
    });
...
const endPremiere = async (req, res) => {
  try {
    ...
    // Any admin, or the original creator, can end a premiere. (Mirrors the
    // socket admin-end-premiere check, so the two paths stay consistent.)
    const isAdmin = req.user.role === 'admin';
    const isCreator = premiere.createdBy?._id?.toString?.() === req.user.id?.toString?.();
    if (!isAdmin && !isCreator) {
      return res.status(403).json({
        success: false,
        message: 'Not authorized to end this premiere'
      });
    }
    ...
    // Broadcast the same events the socket admin-end-premiere path emits so
    // every viewer's LivePremiere modal closes in real time and the card is
    // removed from the home grid.
    const io = req.app.get('io');
    if (io) {
      io.to(`premiere-${premiere._id.toString()}`).emit('premiere-ended', { premiere });
      io.emit('premiere-status-updated', {
        premiereId: premiere._id.toString(),
        action: 'ended',
        premiere,
