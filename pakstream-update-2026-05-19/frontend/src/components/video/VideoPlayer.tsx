...
interface VideoPlayerProps {
  ...
  loop?: boolean; // Restart from 0 when the stream ends (used for live premieres)
  // Passive/live-broadcast mode: hide the local play/pause button and block
  // Picture-in-Picture so viewers can't drive playback independently of the
  // admin. The admin in the premiere control page remains the only driver.
  liveMode?: boolean;
  className?: string;
  onClose?: () => void;
  onPlay?: () => void;
...
const VideoPlayer = forwardRef<VideoPlayerRef, VideoPlayerProps>(({
  ...
  // Keep loop intent reachable from the `ended` listener without
  // re-binding listeners every time the prop changes.
  ...
  useEffect(() => {
    ...
    const initializeVideo = async () => {
      try {
        ...
        // For live premieres the stream must keep running until the admin (or
        // the auto-end scheduler at endTime) explicitly ends it. The native
        // `loop` attribute alone is unreliable with HLS.js because the early
        // segments may have been evicted from the back-buffer by the time we
        // hit the end, so we also explicitly seek to 0 and resume.
        const handleEnded = () => {
    ...
    <>
      ...
      >
        <video
          ...
          autoPlay={autoPlay}
          loop={loop}
          // In live broadcast mode, prevent Picture-in-Picture and any native
          // download/remote-playback affordances. Browsers render PiP windows
          // with separate Play / 10s-back / 10s-forward buttons that let the
          // viewer seek independently of the admin's synced clock.
          {...(liveMode
            ? {
          ...
          <div className={`absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black via-black/80 to-transparent transition-opacity duration-300 ${showControls ? 'opacity-100' : 'opacity-0'}`}>
            ...
            <div className="flex items-center justify-between text-white px-4 pb-4">
              <div className="flex items-center space-x-3">
                {/* In live broadcast mode the admin controls playback; viewers
                    don't get a local play/pause toggle. */}
                {!liveMode && (
                  <button
