...
const sanitizePatchTitle = (value: string) => value.replace(/[^a-zA-Z0-9\s-]/g, '');
  ...
  <div className="space-y-6">
    ...
    >
      <table className="w-full min-w-[1100px] table-auto">
        ...
        <tbody className="divide-y" style={{ borderColor: 'var(--color-border)' }}>
          {loading ? (
            <tr>
              <td colSpan={7} className="px-6 py-10 text-center" style={{ color: 'var(--color-text-secondary)' }}>
                Loading patches...
              </td>
            </tr>
          ) : patches.length === 0 ? (
            ...
            patches.map((patch) => (
              <tr key={patch._id} className="hover:bg-black/5 transition-colors align-top">

                {/* Title */}
                <td className="px-6 py-4">
                  <div className="text-sm font-bold line-clamp-1 max-w-xs" style={{ color: 'var(--color-text)' }} title={patch.title}>
                    {patch.title}
                  </div>
                  <div className="text-xs line-clamp-2 max-w-xs" style={{ color: 'var(--color-text-secondary)' }} title={patch.description}>
                    {patch.description}
                  </div>
                </td>
                ...
                {/* Views column intentionally removed — analytics still
                    consumes patch.views from the API, this is just UI. */}
    ...
    />
                  ...
                  <tbody className="divide-y" style={{ borderColor: 'var(--color-border)' }}>
                    {filteredPatchesForVerification.length === 0 ? (
                      ...
                      filteredPatchesForVerification.map((patch) => (
                        <tr key={patch._id} className="hover:bg-black/5 transition-colors align-top">
                          <td className="px-6 py-4">
                            <div className="font-semibold text-sm line-clamp-1 max-w-xs" style={{ color: 'var(--color-text)' }} title={patch.title}>{patch.title}</div>
        ...
        <ConfirmationDialog
          ...
          title="Delete Patch"
          message="Are you sure you want to delete this patch? This action cannot be undone."
          confirmText="Delete"
          cancelText="Cancel"
          type="danger"
          onConfirm={confirmDelete}
          onCancel={() => setDeleteConfirm({ isOpen: false, patchId: null })}
        />
      </div>
    </ProtectedRoute>
  );
          ...
          <div>
            ...
            <div className="flex space-x-2 mb-2">
              ...
              <button
                ...
                disabled={tagsArr.length >= MAX_TAGS}
              >
                Add
              </button>
            </div>
