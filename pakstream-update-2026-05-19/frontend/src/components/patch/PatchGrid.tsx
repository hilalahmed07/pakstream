        ...
        {patches.map((patch) => (
          ...
          >
            ...
            <div className="p-6 flex-grow">
              ...
              {/* Stats — view count intentionally hidden in the UI; the
                  backend still increments patch.views so analytics is
                  unaffected. Like icon mirrors DocumentGrid / PresentationGrid
                  so the visual treatment is consistent across asset types. */}
              <div className="flex items-center gap-4 mb-4 text-sm" style={{ color: 'var(--color-text-secondary)' }}>
                ...
                <button
                  ...
                  className={`flex items-center transition-colors ${
                    patch.isLiked ? 'text-red-500' : 'hover:text-red-500'
                  }`}
                  style={!patch.isLiked ? { color: 'var(--color-text-secondary)' } : undefined}
                  title={patch.isLiked ? 'Unlike' : 'Like'}
                >
                  <svg
                    className={`w-4 h-4 ${patch.isLiked ? 'fill-current' : ''}`}
                    fill={patch.isLiked ? 'currentColor' : 'none'}
                    stroke="currentColor"
                    viewBox="0 0 24 24"
                  >
                    <path d="M12 21.35l-1.45-1.32C5.4 15.36 2 12.28 2 8.5 2 5.42 4.42 3 7.5 3c1.74 0 3.41.81 4.5 2.09C13.09 3.81 14.76 3 16.5 3 19.58 3 22 5.42 22 8.5c0 3.78-3.4 6.86-8.55 11.54L12 21.35z"/>
                  </svg>
                  <span className="ml-1">{patch.likes || 0}</span>
                </button>
              </div>
