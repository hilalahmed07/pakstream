...
const AdminPremiereDashboard: React.FC = () => {
  ...
  // Premiere id currently being ended. Used to disable the End/Delete buttons
  // and hide the banner immediately so the admin can't re-click while the
  // socket round-trip is in flight.
  const [endingPremiereId, setEndingPremiereId] = useState<string | null>(null);
  ...
  const handleEndPremiere = async (premiereId: string) => {
    if (endingPremiereId === premiereId) return; // Already in flight, ignore double-clicks.
    const snapshot = activePremiere;
    try {
      setEndingPremiereId(premiereId);
      // Hide the LIVE PREMIERE banner immediately so the End/Delete buttons
      // can't be clicked again while the request is in flight. We roll this
      // back on failure.
      setActivePremiere(prev => (prev?._id === premiereId ? null : prev));
      // Use REST instead of the socket emit: it goes through the standard
      // authenticateToken middleware, so authorization doesn't depend on
      // whether the socket handshake happened to carry a valid JWT. The
      // backend still broadcasts `premiere-ended` to viewers via io.
      await premiereService.endPremiere(premiereId);
      showSuccess('Premiere ended');
    } catch (error) {
      // Rollback the optimistic clear so the admin can retry.
      setActivePremiere(snapshot);
          ...
          <div className="mb-8 p-6 rounded-lg" style={{ background: 'linear-gradient(to right, rgba(185, 28, 28, 0.3), rgba(220, 38, 38, 0.3))', border: '1px solid rgb(239, 68, 68)' }}>
            <div className="flex items-center justify-between">
              <div>
                ...
                <h3 className="text-xl font-semibold line-clamp-1 max-w-xl" style={{ color: 'var(--color-text)' }} title={activePremiere.title}>{activePremiere.title}</h3>
                <p className="line-clamp-2 max-w-xl" style={{ color: 'var(--color-text-secondary)' }} title={activePremiere.description}>{activePremiere.description}</p>
              ...
              <div className="flex space-x-2">
                <button
                  ...
                  disabled={endingPremiereId === activePremiere._id}
                  className="bg-red-600 hover:bg-red-700 disabled:opacity-60 disabled:cursor-not-allowed text-white px-4 py-2 rounded-lg font-medium transition-colors"
                >
                  {endingPremiereId === activePremiere._id ? 'Ending…' : 'End Premiere'}
                </button>
                <button
                  onClick={() => handleDeletePremiere(activePremiere._id)}
                  disabled={endingPremiereId === activePremiere._id}
                  className="px-4 py-2 rounded-lg border text-red-400 border-red-400 hover:bg-red-400/10 disabled:opacity-60 disabled:cursor-not-allowed font-medium transition-colors"
                >
                  Delete
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {premieres.filter(premiere => premiere.video && premiere.video.processedFiles).map((premiere) => (
                ...
                >
                  ...
                  <div className="p-4">
                    <h3 className="text-lg font-semibold mb-2 line-clamp-1" style={{ color: 'var(--color-text)' }} title={premiere.title}>{premiere.title}</h3>
                    <p className="text-sm mb-3 line-clamp-2" style={{ color: 'var(--color-text-secondary)' }} title={premiere.description}>{premiere.description}</p>
                    ...
                    <div className="flex space-x-2">
                      ...
                      <button
                        ...
                        className="px-3 py-1 rounded border text-red-400 border-red-400 hover:bg-red-400/10 text-xs font-medium transition-colors"
                      >
                        Delete
                      </button>
                    </div>
