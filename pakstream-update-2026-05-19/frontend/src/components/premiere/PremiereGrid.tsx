      ...
      {premieres.map((premiere) => (
        ...
        >
          ...
          <div className="relative aspect-video bg-secondary overflow-hidden">
            ...
            {/* Time / Status Badge — LIVE for in-flight premieres, otherwise
                the countdown to the scheduled start. */}
            <div className="absolute top-3 left-3">
              {premiereService.isPremiereLive(premiere) ? (
          ...
          {/* Premiere Info */}
          <div className="p-4">
            <h3 className="text-lg font-semibold text-text-primary mb-2 line-clamp-1 group-hover:text-accent transition-colors" title={premiere.title}>
              {premiere.title}
            </h3>

            {premiere.description && (
              <p className="text-text-secondary text-sm mb-3 line-clamp-2" title={premiere.description}>
                {premiere.description}
              </p>
            )}
