...
class PremiereService {
  ...
  // Premieres are admin-controlled and open-ended — status on the server is
  // the single source of truth. We don't gate on endTime any more (the video
  // loops past it) and we don't gate "scheduled" on startTime (the scheduler
  // / getActivePremiere flip the status as soon as start time arrives).
  isPremiereLive(premiere: Premiere): boolean {
    return premiere.status === 'live';
  }

  isPremiereScheduled(premiere: Premiere): boolean {
    return premiere.status === 'scheduled';
  }
