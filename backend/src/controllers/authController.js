...
// Server-side logout: clear the user's sessionToken so the JWT can never
// authenticate again, even if leaked. Without this, removing the token
// client-side leaves the JWT valid until expiry — anyone holding a copy
// could keep using it.
const logout = async (req, res) => {
  try {
    const userId = req.user._id;
    // Null the sessionToken so the auth middleware's match check fails for
    // every JWT this user previously held (including the one being used
    // for this very request — that's fine; we're logging out).
    await User.updateOne(
      { _id: userId },
      { $set: { sessionToken: null } }
    );

    // Drop any live sockets the user has open (in any browser/tab) so the
    // realtime layer disconnects in lock-step with the JWT becoming
    // invalid. Same plumbing as concurrent-login eviction.
    const socketHandler = req.app.get('socketHandler');
    if (socketHandler && typeof socketHandler.evictUserSessions === 'function') {
      socketHandler.evictUserSessions(userId);
    }
