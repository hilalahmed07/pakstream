    ...
    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
      {videos.map((video) => {
        ...
        <div key={video._id} className="bg-card rounded-lg overflow-hidden group hover:scale-105 hover:bg-card-hover transition-all">
          ...
          <div className="p-4">
            <h3 className="text-text-primary font-semibold mb-2 line-clamp-1" title={video.title}>{video.title}</h3>
            <p className="text-text-secondary text-sm mb-3 line-clamp-2" title={video.description}>{video.description}</p>
