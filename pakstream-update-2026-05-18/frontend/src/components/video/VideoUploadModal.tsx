...
const VideoUploadModal: React.FC<VideoUploadModalProps> = ({ 
  ...
  const tagsArr = uploadData.tags.split(',').map(t => t.trim()).filter(Boolean);
  ...
  const addTag = () => {
    ...
    if (!tagsArr.includes(nextTag)) {
      setUploadData(prev => ({ ...prev, tags: [...tagsArr, nextTag].join(',') }));
          ...
          <div>
            ...
            <div className="flex space-x-2 mb-2">
              ...
              <button
                ...
                disabled={isUploading || tagsArr.length >= MAX_TAGS}
              >
                Add
              </button>
            </div>
