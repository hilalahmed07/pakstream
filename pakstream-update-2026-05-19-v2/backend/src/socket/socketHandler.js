...
class SocketHandler {
  constructor(server) {
    ...
    // Track the active socket per userId (admin OR regular user) so a second
    // login can gracefully evict the previous session from any other
    // browser/tab.
  ...
  setupSocketHandlers() {
      ...
      try {
        ...
        if (token) {
          try {
            ...
            const user = await User.findById(decoded.userId).select('username email role sessionToken');

            // Reject sockets carrying a stale session token (a newer login
            // has rotated the user's sessionToken). The client will detect
            // the disconnect/force-logout and clear local auth.
            if (
              user &&
              user.isActive &&
            ...
            } else if (user && decoded.sessionToken && decoded.sessionToken !== user.sessionToken) {
              console.log('Socket rejected: stale sessionToken for', user.username);
            }
          } catch (error) {
    ...
    this.io.on('connection', (socket) => {
      ...
      // Put every authenticated socket into a per-user room so the login
      // endpoint can broadcast `force-logout` to every other browser/tab for
      // the same account without having to track socket IDs by hand.
      if (socket.userId) {
        socket.join(`user:${socket.userId}`);

        // Belt-and-braces: if this user already has another socket open
        // (race between login finishing and the new browser reconnecting its
        // socket), evict that older session here too.
        const existingSocketId = this.userSessions.get(socket.userId);
          ...
          if (existingSocket) {
            ...
            setTimeout(() => {
              try {
                ...
                console.error('Error disconnecting previous user session:', e);
              }
            }, 500);
          }
        }
      ...
      socket.on('join-premiere', async (premiereId) => {
        try {
          ...
          // This simulates a TV broadcast — viewers joining in the middle get
          // the live current time. The premiere loops until the admin ends it
          // (or the scheduler auto-ends at endTime), so we wrap the elapsed
          // time with modulo of the video duration instead of clamping it.
          const elapsedTime = Math.max(0, (new Date() - roomData.startTime) / 1000); // seconds
          const videoDuration = premiere.video?.duration || 0;
          const currentPlaybackTime = videoDuration > 0
      ...
      socket.on('admin-start-premiere', async (premiereId) => {
        try {
          ...
          // QC scenarios (start→end within <1s) could fire this against a
          // torn-down room; guard so the delayed emit only happens if the
          // room is still live, and shield against any unexpected throw.
          setTimeout(() => {
            try {
              const room = this.premiereRooms.get(premiereId);
              if (!room || !room.isLive) return;
              this.io.to(`premiere-${premiereId}`).emit('video-play');
            } catch (e) {
              console.error('Delayed video-play emit failed:', e);
            }
          }, 1000);
      ...
      socket.on('disconnect', () => {
        ...
        // Release the user session slot only if this socket still owns it.
        // (A newer login may have already overwritten the entry — don't
        // accidentally delete the active session in that case.)
        if (socket.userId && this.userSessions.get(socket.userId) === socket.id) {
  ...
  // Force-logout every currently connected socket belonging to a user.
  // Called from the login controller right after the user's sessionToken is
  // rotated — this guarantees the previous browser/device gets the eviction
  // event in real time, regardless of when the new browser opens its socket.
  evictUserSessions(userId) {
    ...
    this.io.to(room).emit('force-logout', {
      reason: 'concurrent-login',
      message:
        'Your account has been signed in on another device or browser. For your security, this session has been ended.'
    });

    // Disconnect after a short delay so the client can render the notice.
    setTimeout(() => {
      try {
        this.io.in(room).disconnectSockets(true);
      } catch (e) {
        console.error('Error disconnecting previous user sessions:', e);
      }
    }, 500);
  }
