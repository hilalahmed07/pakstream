...
const AppContent: React.FC = () => {
  ...
  const [forceLogoutMessage, setForceLogoutMessage] = React.useState<string | null>(null);
  ...
  // Server-driven sign-out: another browser/tab signed in with this
  // account, so the backend evicts this session. Two upstream channels feed
  // into the same `force-logout` window event:
  //   1. socketService re-dispatches the socket `force-logout` payload as a
  //      window event (works in real time across socket reconnects).
  //   2. The global fetch interceptor dispatches it when any REST call comes
  //      back with code === 'SESSION_INVALIDATED' (fallback for sessions
  //      whose socket isn't alive).
  React.useEffect(() => {
    ...
    window.addEventListener('force-logout', windowHandler);
    return () => {
      window.removeEventListener('force-logout', windowHandler);
    };
  }, [logout]);
  ...
  return (
    <div className="min-h-screen bg-primary flex flex-col">
      ...
      <ConfirmationDialog
        isOpen={forceLogoutMessage !== null}
        ...
        confirmText="OK"
        onConfirm={() => setForceLogoutMessage(null)}
        onCancel={() => setForceLogoutMessage(null)}
        type="warning"
        hideCancel
      />
    </div>
